...
function Popover({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className="absolute z-[1200] mt-2 rounded-lg border border-border bg-surface p-4 shadow-pop"
          style={{ width }}
        >
          {children}
        </div>
...
export function SitesToolbar({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // (supply controls are "active" when changed from defaults — see compute/defaults.ts)
  ...
  return (
    <div
      className={cn(
        ...
        embedded
          ? "" // inline: flows in the map's control row
          : "sticky top-0 z-20 mb-4 rounded-lg border border-border bg-surface/95 px-3 py-2 shadow-sm backdrop-blur",
      )}
    >
